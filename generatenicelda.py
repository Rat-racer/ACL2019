...
print("Loaded %d papers from papers.pickle (generated by scrape.py)" % (len(paperdict), ))

# load the top word frequencies (result of pdftowordcloud.py)
topdict = pickle.load(open("topwords.pickle", "rb"))
print("Loaded %d entries from topwords.pickle (generated by pdftowordcloud.py)" % (len(topdict), ))

# load LDA words and invert their dictionary list
(ldak, phi, voca) = pickle.load(open("ldaphi.pickle", "rb"))
wtoid = {}
...
html = open("aclnice_template.html", "r").read()
s = ""
js = "ldadist=["
js2 = "pairdists=["
...
for pid, p in enumerate(paperdict):
	...
	print("PROCESSING:", p, '/', len(paperdict))
	
	# get title, author, bib
	#title, author, bib, supp = paperdict[p]
	pdflink = paperdict[p][0]
	title = paperdict[p][1]
	abstract = paperdict[p][3]
	author = ', '.join(paperdict[p][4].keys())
	bib = paperdict[p][-1]
	supp = ' ' 
	...
	# get links to PDF on ICCV servers
	#pdflink = "http://openaccess.thecvf.com/content_ICCV_2019/papers/%s_ICCV_2019_paper.pdf" % (p, )
	...
	""" % (pid, # main div id
		   ...
		   thumbpath, # image
		   pid, # abs display num
		   abstract,#link_url(open('abstracts/%s_ICCV_2019_paper.txt' % (p,), 'r').read()), # abstract
